...
const Themes = () => {
  ...
  const themeInfo = {
    default: {
      ...
      description: 'A clean, professional theme for general healthcare services.',
      features: [
    ...
    organTransplant: {
      ...
      description: 'Specialized theme for organ transplantation services.',
      features: [
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <Card 
        ...
        header={<h4 className="text-lg">Current Theme: {currentTheme.title}</h4>}
      >
        <div className="p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {Object.entries(themeInfo).map(([key, theme]) => (
          <Card 
            key={key}
            className={`${key === specialty ? 'ring-2 ring-offset-2' : ''} ${key === specialty ? (
              specialty === 'organTransplant' ? 'ring-teal-500' :
              specialty === 'cosmeticSurgery' ? 'ring-pink-500' : 'ring-primary'
            ) : ''}`}
            headerClass={key === 'organTransplant' ? 'bg-teal-700 text-white' : 
                        key === 'cosmeticSurgery' ? 'bg-pink-700 text-white' : 
