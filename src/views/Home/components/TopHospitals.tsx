...
import { HiOutlineLocationMarker, HiOutlineCalendar, HiOutlineHeart, HiOutlineStar, HiOutlineShieldCheck } from 'react-icons/hi';
...
const TopHospitals: React.FC<TopHospitalsProps> = ({ hcfData }) => {
    ...
    const getRandomSpecializations = (specializations = [], count = 3) => {
        if (!specializations.length) {
            ...
            const defaults = ['Cardiology', 'Orthopedics', 'Neurology', 'Oncology', 'Pediatrics'];
            ...
            <div className="max-w-7xl mx-auto px-4">
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-600 max-w-2xl mx-auto">
                        Discover world-class healthcare facilities with advanced technology and expert medical staff
                    </p>
                </div>
                            ...
                            <div>
                                <div className="relative h-48 overflow-hidden">
                                    ...
                                    <div className="absolute top-2 right-2 bg-white/80 backdrop-blur-sm rounded-full px-3 py-1 flex items-center gap-1 text-sm font-medium">
                                        <HiOutlineStar className={`${themeClasses.iconColor}`} />
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        {getRandomSpecializations(hospital.specializations, 3).map((specialty, i) => (
                                            <Badge 
                                                key={i} 
                                                className={`${themeClasses.badgeBg} ${themeClasses.badgeText} text-xs font-medium`}
                                            >
                                                {typeof specialty === 'string' ? specialty.split(' ').slice(0, 2).join(' ') : 'Specialty'}
                                            </Badge>
                                        ))}
                                    </div>
                                    ...
                                    <div className="space-y-3 text-gray-600 text-sm mb-4">
                                        <InfoRow
                                            ...
                                            label="Established"
                                            value={hospital.establishedYear?.toString() || 'N/A'}
                                        />
                                        <InfoRow
                                    ...
                                    {getFeaturedTreatments(hospital).length > 0 && (
                                        <div className="mt-4 border-t pt-3">
                                            <h3 className="text-sm font-medium text-gray-700 mb-2 flex items-center">
                                                <MdOutlineLocalHospital className={`mr-1 ${themeClasses.iconColor}`} />
                                                Featured Treatments
                                            </h3>
                                            <ul className="space-y-1">
                                                {getFeaturedTreatments(hospital).map((treatment, i) => (
