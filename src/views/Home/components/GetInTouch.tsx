...
import { HiOutlineMail, HiOutlineLocationMarker, HiOutlineGlobe } from 'react-icons/hi';
...
const GetInTouch: React.FC<GetInTouchProps> = ({ hcfData }) => {
    ...
    return (
        <div className={`w-full ${themeClasses.sectionBg} py-16 md:py-20 transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            <div className="max-w-7xl mx-auto px-6">
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-600 max-w-2xl mx-auto">
                        Have questions about treatments or need personalized assistance? Our expert team is ready to help you on your healthcare journey.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        <div className={`${themeClasses.cardBg} rounded-xl p-8 shadow-md border ${themeClasses.cardBorder} ${themeClasses.cardHover} transition-all duration-300`}>
                            <div className="flex flex-wrap gap-4 mb-6 items-center">
                                <h3 className="text-2xl font-semibold text-gray-900">{hcfData?.name || 'Healthcare Center'}</h3>
                                <div className="flex gap-2">
                                    {(hcfData?.languages?.length ? hcfData?.languages : ['English']).map((lang, idx) => (
                                        <Badge 
                                            key={idx} 
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-5">
                                    <a href={`tel:${hcfData?.phone || hcfData?.phoneNumber}`} className={`flex items-center gap-3 text-gray-700 hover:${themeClasses.iconColor} transition-colors group`}>
                                        <div className={`p-3 rounded-full ${themeClasses.iconBg} ${themeClasses.iconColor} group-hover:scale-105 transition-transform`}>
                                            <BiPhone className="w-5 h-5" />
                                        </div>
                                        <span>{hcfData?.phone || hcfData?.phoneNumber || '+1 (555) 123-4567'}</span>
                                    </a>

                                    <a href={`mailto:${hcfData?.auth?.email || hcfData?.email}`} className={`flex items-center gap-3 text-gray-700 hover:${themeClasses.iconColor} transition-colors group`}>
                                        <div className={`p-3 rounded-full ${themeClasses.iconBg} ${themeClasses.iconColor} group-hover:scale-105 transition-transform`}>
                                            <HiOutlineMail className="w-5 h-5" />
                                        </div>
                                        <span>{hcfData?.auth?.email || hcfData?.email || 'contact@healthcare.com'}</span>
                                    </a>

                                    <div className={`flex items-start gap-3 text-gray-700 group`}>
                                        <div className={`p-3 rounded-full ${themeClasses.iconBg} ${themeClasses.iconColor} group-hover:scale-105 transition-transform mt-1 flex-shrink-0`}>
                                            <HiOutlineLocationMarker className="w-5 h-5" />
                                        </div>
                                        <span className="line-clamp-3">
                                            {hcfData?.address?.street && `${hcfData.address.street}, `}
                                            {hcfData?.address?.city && `${hcfData.address.city}, `}
                                            {hcfData?.address?.country || 'Location Available Upon Request'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-5">
                                    <div className={`flex items-center gap-3 text-gray-700`}>
                                        <div className={`p-3 rounded-full ${themeClasses.iconBg} ${themeClasses.iconColor}`}>
                                            <HiOutlineGlobe className="w-5 h-5" />
                                        </div>
                                        <span>{hcfData?.website || 'www.healthcarewebsite.com'}</span>
                                    </div>
                                    
                                    <div className="space-y-3">
                                        <h4 className="font-semibold text-gray-900 flex items-center gap-2">
                                            <span className={`${themeClasses.iconColor}`}>•</span>
                                            Connect with us
                                        </h4>
                                        <div className="flex gap-3">
                                            <a href="#" className={`p-2.5 rounded-full ${themeClasses.iconBg} ${themeClasses.iconColor} hover:bg-gradient-to-r hover:${themeClasses.buttonGradient} hover:text-white transition-all duration-300`}>
                    ...
                    <div className="lg:col-span-2">
                        <div className={`${themeClasses.cardBg} rounded-xl p-8 shadow-md border ${themeClasses.cardBorder} h-full flex flex-col`}>
                            ...
                            <div className={`mt-6 p-3 rounded-lg ${themeClasses.accentBg} text-sm ${themeClasses.iconColor} flex items-start gap-2`}>
